    ...
    <React.Fragment>
      ...
      >
        ...
        {hasSearched && availableLocksmiths.map((locksmith) => {
          ...
          // Get bearing from road direction if available, otherwise use a fallback
          ...
          const snappedPoint = roadSnappedPoints[`${locksmith.id}-current`];
          if (snappedPoint && typeof snappedPoint.bearing === 'number') {
            // Use the road bearing
            bearing = snappedPoint.bearing;
            console.log(`Company: ${locksmith.companyName}, Road Bearing: ${bearing}°`);
          } else {
            // Fallback: Assign different bearings based on company name hash
            const companyNameHash = locksmith.companyName.split('').reduce(
              (acc, char) => acc + char.charCodeAt(0), 0
            );
          ...
          if (currentLocation) {
            // Store current location for future real movement tracking
          ...
          return (
            <div key={locksmith.id}>
              ...
              {currentLocation && isVanVisible ? (
                /* If tradesperson is sharing live location, show van icon */
                <Marker
                ...
                >
                  ...
                  >
                    ...
                    <div key={`van-container-${tradespersonId}-${Date.now()}-${bearing}`}>
                      <VanIcon3D 
                        className="h-10 w-10"
                        bearing={bearing} 
                        animate={true}
              ...
              ) : hqLocation && isHQVisible ? (
                /* If no live location but HQ exists, show shop icon */
                <Marker
                  key={`${locksmith.id}-hq`}
                  latitude={hqLocation.latitude}
                  longitude={hqLocation.longitude}
